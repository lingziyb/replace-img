...
const regex = /\!\[.*\]\((http.*)\)/;
const postDirPath = path.resolve(__dirname, "./../example");
...
function saveImg(dirName, imgFileName, imgData) {
  ...
  fs.writeFileSync(`${dirPath}/${imgFileName}.png`, imgData, "binary");
}

function replace(filePath, imgFileName, url) {
  const fileData = fs.readFileSync(filePath, "utf8");
  const newFile = fileData.replace(url, `./${imgFileName}.png`);
